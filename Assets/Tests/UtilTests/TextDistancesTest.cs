...
public class TextDistancesTest
{
    ...
    [Test]
    public void GetRewardAfterProofreading_OneRemainingErrorAfterUpdate()
    {
    ...
    [Test]
    public void GetRewardAfterProofreading_TwoRemainingErrorAfterUpdate()
    {
        string previousBeliefText = "She bakes breattt";
        string updatedBeliefText = "She bakes breatt";
        string correctText = "She bakes bread";
    ...
    [Test]
    public void GetRewardAfterProofreading_Correction()
    {
        string previousBeliefText = "Ss";
        string updatedBeliefText = "S";
        string correctText = "She bakes bread";
    ...
    [Test]
    public void CalculateLevenshtein_MultipleEdits_ShouldReturnEditCount()
    {
        string target = "kitten";
        string current = "sitting";
        int result = TextDistance.CalculateLevenshtein(target.ToLower(), current.ToLower());
        Assert.AreEqual(3, result); // 3 edits: 'k' -> 's', 'e' -> 'i', append 'g'
    }

    [Test]
    public void CalculateLevenshtein_OneStringContainsOther_ShouldReturnDifferenceInLength()
    {
        string target = "flaw";
        string current = "flaws";
        int result = TextDistance.CalculateLevenshtein(target.ToLower(), current.ToLower());
        Assert.AreEqual(1, result); // Adding one character
    }

    [Test]
    public void CalculateLevenshtein_OneCharacterDifference_ShouldReturnOne()
    {
        string target = "cat";
        string current = "bat";
        int result = TextDistance.CalculateLevenshtein(target.ToLower(), current.ToLower());
        Assert.AreEqual(1, result); // Because replacing 'c' with 'b' is needed
    }

    [Test]
    public void CalculateLevenshtein_OneStringEmpty_ShouldReturnLengthOfNonEmptyString()
    {
        string target = "abc";
        string current = "";
        int result = TextDistance.CalculateLevenshtein(target.ToLower(), current.ToLower());
        Assert.AreEqual(3, result); // Because deleting all 3 characters is needed
    }

    [Test]
    public void CalculateLevenshtein_CompletelyDifferentStrings_ShouldReturnLengthOfLongestString()
    {
        string target = "abc";
        string current = "xyz";
        int result = TextDistance.CalculateLevenshtein(target.ToLower(), current.ToLower());
        Assert.AreEqual(3, result); // Because replacing all 3 characters is needed
    }

    [Test]
    public void CalculateLevenshtein_IdenticalStrings_ShouldReturnZero()
    {
        string target = "hello";
    ...
    [Test]
    public void CalculateLevenshtein_Sentence()
    {
        string target = "hello from the other side";
        string current = "hello fro the otterside";
        int result = TextDistance.CalculateLevenshtein(target.ToLower(), current.ToLower());
        Assert.AreEqual(3, result);
    }

    [Test]
    public void CalculateAccuracy_Sentence()
    {
        string target = "my name is patrick";
        string current = "myy name is patrick";
        double result = TextDistance.CalculateAccuracy(target.ToLower(), current.ToLower());
        Assert.AreEqual(0.9444, result, 0.01);
    }

    [Test]
    public void CalculateAccuracy_MulitpleMistakes()
    {
        string target = "my name is patrick";
        string current = "myy name is patryc";
        double result = TextDistance.CalculateAccuracy(target.ToLower(), current.ToLower());
        Assert.AreEqual(0.8333, result, 0.01);
    }

    [Test]
    public void CalculateAccuracy_EmptyStrings()
    {
        string target = "";
        string current = "";
        double result = TextDistance.CalculateAccuracy(target.ToLower(), current.ToLower());
