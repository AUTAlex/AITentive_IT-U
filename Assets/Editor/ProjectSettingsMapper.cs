...
public static class ProjectSettingsMapper
{
    public static void JsonSettingsToProjectSettings(JsonSettings settings, IProjectSettings projectSettings)
    {
        ...
        foreach (var member in settings.members)
        {
            ...
            if (memberInfo != null)
            {
                var result = ConvertStringToObject(member.Value, memberInfo.GetUnderlyingType());
    ...
    public static void HyperparametersToProjectSettings(Dictionary<Type, ISettings> settings, IProjectSettings projectSettings)
    {
        ...
        SupervisorAgent supervisorAgent = GetSupervisorAgent(projectSettings, supervisorSettings);
        ...
        if (hyperparameters.tasks.Length > 1)
        {
        ...
        VisualTaskQueueChoice.TryParse(hyperparameters.visualTaskQueueChoice, out VisualTaskQueueChoice visualTaskQueueChoice);
        projectSettings.VisualTaskQueueChoice = hyperparameters.visualTaskQueueChoice != null ? visualTaskQueueChoice : projectSettings.VisualTaskQueueChoice;
        projectSettings.UseAttentionTimer = hyperparameters.useAttentionTimer != null ? hyperparameters.useAttentionTimer.GetValueOrDefault() : projectSettings.UseAttentionTimer;
        projectSettings.UseVisibilityTimer = hyperparameters.useVisibilityTimer != null ? hyperparameters.useVisibilityTimer.GetValueOrDefault() : projectSettings.UseVisibilityTimer;
        ...
        projectSettings.VisionAgentChoice = GetVisonAgentChoice(hyperparameters);
        projectSettings.UseVisionAgentPerTask = hyperparameters.useVisionAgentPerTask.GetValueOrDefault();
        projectSettings.UseConstantAspectRatio = hyperparameters.useConstantAspectRatio.GetValueOrDefault();
        projectSettings.HeuristicModeForVisionAgent = hyperparameters.heuristicModeForVisionAgent != null ? hyperparameters.heuristicModeForVisionAgent.GetValueOrDefault() : projectSettings.HeuristicModeForVisionAgent;

        AssignValuesToTasks(hyperparameters, projectSettings);
    }
    ...
    public static void PerformanceMeasurementSettingsToProjectSettings(PerformanceMeasurementSettings performanceMeasurementSettings, IProjectSettings projectSettings)
    {
        ...
        performanceMeasurement.MinimumDurationForMeasurement = performanceMeasurementSettings.minimumScoreForMeasurement != null ? performanceMeasurementSettings.minimumScoreForMeasurement.GetValueOrDefault() : performanceMeasurement.MinimumDurationForMeasurement;
    ...
    public static void ExperimentSettingsToProjectSettings(ExperimentSettings experimentSettings, SupervisorSettings supervisorSettings, IProjectSettings projectSettings)
    {
        ...
        projectSettings.MultiScreen = experimentSettings.multiScreen.GetValueOrDefault();
    ...
    private static SupervisorAgent GetSupervisorAgent(IProjectSettings projectSettings, SupervisorSettings supervisorSettings)
    {
        string supervisorChoice = GetSupervisorChoice(supervisorSettings).ToString();
    ...
    private static SupervisorChoice GetSupervisorChoice(SupervisorSettings supervisorSettings)
    {
        ...
        else if (supervisorSettings.supervisorChoice != null)
        {
    ...
    private static VisionAgentChoice GetVisonAgentChoice(Hyperparameters hyperparameters)
    {
        if (hyperparameters.visionAgentChoice != null)
        {
            VisionAgentChoice.TryParse(hyperparameters.visionAgentChoice, out VisionAgentChoice visionAgentChoice);
            return visionAgentChoice;
        }
        else
        {
            return VisionAgentChoice.FocusAgent;
        }
    }
    ...
    private static void AssignValuesToTasks(Hyperparameters hyperparameters, IProjectSettings projectSettings)
    {
        foreach (ITask task in projectSettings.Tasks)
        {
            if (task.GetType().GetInterfaces().Contains(typeof(ICRTask)))
            {
                ((ICRTask)task).UseVisionAgent = hyperparameters.useFocusAgent != null ? hyperparameters.useFocusAgent.GetValueOrDefault() : ((ICRTask)task).UseVisionAgent;
            }
            task.IsAutonomous = hyperparameters.autonomous != null ? hyperparameters.autonomous.GetValueOrDefault() : task.IsAutonomous;
        }
    }

    private static object ConvertStringToObject(string value, Type type)
    {
    ...
    {
        return typeof(ScriptableObject).IsAssignableFrom(type) && !type.IsAbstract;
    }
}
