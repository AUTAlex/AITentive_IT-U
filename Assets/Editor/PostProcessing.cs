...
public class PostProcessing
{
    ...
    public static void EnrichModels()
    {
        ...
        CreateAITentiveModel(modelPath, settings, "FocusAgent", "Focus");
    ...
    private static void EnrichTaskModels(string modelPath, Dictionary<Type, ISettings> settings)
    {
        ...
        foreach (string task in ((Hyperparameters)settings[typeof(Hyperparameters)]).tasks)
        {
            ...
            string baseType = Util.GetType(task).GetLowestBaseTypeInHierarchyOf(typeof(Agent)) == typeof(Task) ? Util.GetType(task).GetLowestBaseTypeInHierarchyOf(typeof(Task)).Name : Util.GetType(task).GetLowestBaseTypeInHierarchyOf(typeof(Agent)).Name;
    ...
    private static void CreateAITentiveModel(string modelPath, Dictionary<Type, ISettings> settings, string baseType, string onnxFile = null)
    {
        ...
        ModelAsset asset = AssetDatabase.LoadAssetAtPath(newPath, typeof(ModelAsset)) as ModelAsset;
        ...
        if (settings.ContainsKey(typeof(SupervisorSettings)))
        {
            ...
            aITentiveModel.DecisionPeriod = baseType == "SupervisorAgent" ? aITentiveModel.SupervisorSettings.decisionPeriod.GetValueOrDefault() : aITentiveModel.DecisionPeriod;
        }
