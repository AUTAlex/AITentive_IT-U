...
public class BuildScript
{
    ...
    public static void BuildEvaluationEnvironment()
    {
        ...
        string comparisonFileName = args.Count >= 6 ? args[5] : "";
        bool overwriteOldEvaluation = args.Contains("-o") ? true : false;
        string[] parts = confFile.Split('.');
        Assert.AreEqual("json", parts[parts.Length - 1]);

        if (ballAgentModelName != " ")
        {
        ...
        if (evaluationSettings.ContainsKey(typeof(BehavioralDataCollectionSettings)))
        { 
            settings[typeof(BehavioralDataCollectionSettings)] = evaluationSettings[typeof(BehavioralDataCollectionSettings)];
        }
        else
        {
            evaluationSettings[typeof(BehavioralDataCollectionSettings)] = new BehavioralDataCollectionSettings();
            settings[typeof(BehavioralDataCollectionSettings)] = new BehavioralDataCollectionSettings();
        }
        ...
        if (evaluationSettings.ContainsKey(typeof(BalancingTaskSettings)))
        {
        ...
        ((Hyperparameters)settings[typeof(Hyperparameters)]).timeScale = timeScale;
        ...
        ((Hyperparameters)settings[typeof(Hyperparameters)]).taskModels["BallAgent"] = ballAgentModelName != "" ?  ballAgentModelName : ((Hyperparameters)settings[typeof(Hyperparameters)]).taskModels["BallAgent"];

        BuildEnvironment(settings, buildPath);
    }
    ...
    private static void BuildEnvironment(Dictionary<Type, ISettings> settings, string buildPath, Action<Dictionary<Type, ISettings>> validate = null)
    {
        string workingDirectory = Application.dataPath;
        workingDirectory = workingDirectory.Replace("Assets", ".");
