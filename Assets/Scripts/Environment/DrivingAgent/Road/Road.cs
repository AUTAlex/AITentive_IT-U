...
public readonly struct RoadContext
{
    public readonly int PreviousSpeed;
    public readonly Lane PreviousLane;
    public readonly bool IsSignActiveOnRoad;
    public readonly GameContext GameContext;

    public RoadContext(
        int previousSpeed,
        Lane previousLane,
        bool isSignActiveOnRoad,
        GameContext gameContext
        )
    {
        PreviousSpeed = previousSpeed;
        PreviousLane = previousLane;
        IsSignActiveOnRoad = isSignActiveOnRoad;
        GameContext = gameContext;
    }
}
...
public class Road : MonoBehaviour {
    ...
    public event Action<Road> PlayerEntered;
    ...
    private RoadContext _roadContext;


    public void InitRoadLength(GameContext gameContext) 
    {
        RoadLength = RoadTilesAmount * 10;
        
        //gets min needed road length
        float minRoadLength = 1f * (gameContext.MaxSpeed / 3.6f) + 4.6f * (gameContext.MaxSpeed / 3.6f) + 20f;
    ...
    public void InitRoads(RoadContext roadContext) {
        _roadContext = roadContext;
        Reset();
        PlaceRoadTiles();
        PlaceSpeedSign();
        PlaceRoadSign(roadContext.PreviousLane);
    }
    ...
    public void PlaceSpeedSign() {
        ...
        int steps = ((_roadContext.GameContext.MaxSpeed - _roadContext.GameContext.MinSpeed) / _roadContext.GameContext.Step) + 1;
        int randomIndex = Random.Range(0, steps);
        int randomNumber = _roadContext.GameContext.MinSpeed + (randomIndex * _roadContext.GameContext.Step);
    ...
    public MonoBehaviour GetObjectOnRoad<T>() where T : MonoBehaviour
    {
        ...
        {
            throw new InvalidOperationException($"No property of type {typeof(T)} found.");
        }
    }
    ...
    public void PlaceRoadSign(Lane previousLane, bool isFirstSpawn = true) {
        
        OverheadSign.gameObject.SetActive(_roadContext.IsSignActiveOnRoad);

        if (_roadContext.IsSignActiveOnRoad) {
            
            //Max Possible Length for Lane Switch
            float laneSwitchLengthMaxSpeed = 1f * (_roadContext.GameContext.MaxSpeed / 3.6f) + 4.6f * (_roadContext.GameContext.MaxSpeed / 3.6f);
            ...
            _activeLane = OverheadSign.AssignRandomSign(previousLane);
        } else {
    ...
    {
        OverheadSign.TargetLaneChanged += OnTargetLaneChanged;
        SpeedSign.TargetSpeedChanged += OnTargetSpeedChanged;
    }
    ...
    private void DrawDrivingLanes(Lane previousLane)
    {
        ...
        roadStartPosition.x = DrivingUtil.GetXLocationForLane(previousLane, gameObject);
        signPosition.x = DrivingUtil.GetXLocationForLane(previousLane, gameObject);
        ...
        startPosAfterSign.x = DrivingUtil.GetXLocationForLane(_activeLane, gameObject);
        roadEndPosition.x = DrivingUtil.GetXLocationForLane(_activeLane, gameObject);

        int speedAtSign = signPosition.z > SpeedSign.gameObject.transform.position.z
            ? SpeedSign.SignSpeed
            : _roadContext.PreviousSpeed;
