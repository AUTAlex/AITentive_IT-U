...
public class PixelVisionAgentSequential : VisionAgent, IPixelVisionAgent
{
    ...
    {
        List<string> ignoreTags = Tasks.DistinctBy(x => x.GetType()).SelectMany(x => x.IgnoredTagsForVision).ToList();

        HashSet<GameObject> objectsInFovea = new();
        List<GraphicRaycaster> raycasters = GameObject.FindObjectsByType<GraphicRaycaster>(FindObjectsSortMode.None)
            .Where(x =>
        ...
        objectsInFovea.UnionWith(GetGameObjectsAtPosition(raycasters, _gazePixelPosition));
        int fovealRadiusPixels = (int)GetFovealRadiusPixel(1);

        List<Vector2Int> spiralSamples = GenerateFovealSamplePositions(_gazePixelPosition, SampleCount, fovealRadiusPixels);

        foreach (var samplePos in spiralSamples)
        {
            objectsInFovea.UnionWith(GetGameObjectsAtPosition(raycasters, samplePos));
        }

        return objectsInFovea.ToList();
    }

    public HashSet<GameObject> GetGameObjectsAtPosition(List<GraphicRaycaster> raycasters, Vector2Int pixelPosition)
    {
        Camera renderCam = _cameraInfos[_currentTaskIndex].RenderCamera;
        Camera displayCam = _cameraInfos[_currentTaskIndex].DisplayCamera;

        Vector2 viewport = GetGazeViewport(pixelPosition);
        Ray ray = displayCam.ViewportPointToRay(viewport);

        //Debug.DrawRay(ray.origin, ray.direction * 1000f, Color.magenta, 0.02f);

        int mask = 1 << VISIONLAYER;
        var rawHits = Physics.RaycastAll(ray, 100f, mask);
        var hitObjects = new HashSet<GameObject>();
        foreach (var h in rawHits)
        {
            var go = h.collider.gameObject;
            hitObjects.Add(go);
            hitObjects.UnionWith(go.GetAllParents("SpawnContainer"));
        }

        Vector3 screenPos = displayCam.ViewportToScreenPoint(viewport);
        hitObjects.UnionWith(GetUIElementsAtGaze(raycasters, screenPos, displayCam));
    ...
    private void AddVisionColliders()
    {
        ...
        foreach (var rend in renderers)
        {
            ...
            Vector3 localCenter = rend.transform.InverseTransformPoint(bounds.center);

            // Compute size in local space
            Vector3 worldSize = bounds.size;
            Vector3 localSize = rend.transform.InverseTransformVector(worldSize);

            // Take absolute to ensure positive sizes
    ...
    private Vector2 GetGazeViewportFromRenderTexture(Vector2Int gazePixelPosition)
    {
        RenderTexture renderTexture = GetRenderTextureForCurrentState();

        Vector2 viewport = new Vector2(
            (renderTexture.width / 2 + (float)gazePixelPosition.x) / renderTexture.width,
            (renderTexture.height / 2 + (float)gazePixelPosition.y) / renderTexture.height
        );
    ...
    private Camera CloneCamera(Camera originalCamera, ITask task, RenderTexture renderTexture)
    {
        ...
        targetCamera.GetComponent<PostProcessLayer>().enabled = false;
        targetCamera.GetComponent<PostProcessVolume>().enabled = false;
    ...
    private void KeyboardHeuristic(in ActionBuffers actionsOut)
    {
        ...
        Debug.Log($"result.Item2.x: {result.Item2.x};\t result.Item2.y: {result.Item2.y}");
