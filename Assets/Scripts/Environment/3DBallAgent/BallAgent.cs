...
public abstract class BallAgent : Task, IComparable<BallAgent>, IBallAgent
{
    ...
    public override Dictionary<string, double> AccumulatedPerformance {
        get 
        {
    ...
    public override void OnEpisodeBegin()
    {
        ...
        if (IsAutonomous)
        {
            ...
            _ballRb.linearVelocity = new Vector3(0f, 0f, 0f);
    ...
    {
        return _ballRb != null ? _ballRb.linearVelocity : default;
    }
    ...
    {
        return _ballRb != null ? _ballRb.linearDamping : default;
    }
    ...
    {
        return _ballRb != null ? _ballRb.angularDamping : default;
    }
    ...
    public override void UpdateDifficultyLevel()
    {
        ...
        {
            _ballRb.linearDamping -= _dragDifficulty;
        }
        else
        {
            _ballRb.linearDamping /= _dragDifficulty;
        ...
        Debug.Log(string.Format("Drag: {0}, ForceDifficulty: {1}", _ballRb.linearDamping, ForceDifficulty));
    ...
    {
        if (Vector3.Magnitude(_ballRb.linearVelocity) == 0)
        {
            _ballRb.AddRelativeForce(Vector3.forward * ForceDifficulty);
        }
        else if (Vector3.Magnitude(_ballRb.linearVelocity) < 0.1f)
        {
            _ballRb.AddForce(_ballRb.linearVelocity.normalized * ForceDifficulty);
        }
    }
    ...
    protected virtual void PropagateTaskCompletion()
    {
        if (HasLost())
        {
            EndEpisode();
    ...
    private void ResetEnvironmentParameters()
    {
        ...
        _ballRb.linearVelocity = new Vector3(0f, 0f, 0f);
        _ballRb.linearDamping = GlobalDrag;
