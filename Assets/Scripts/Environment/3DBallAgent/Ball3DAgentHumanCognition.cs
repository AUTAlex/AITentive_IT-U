...
public class Ball3DAgentHumanCognition : BallAgent, ICRTask
{
    ...
    {
        return _ballLocationProbabilities;
    }
    ...
    {
        base.OnActionReceivedInternal(actionBuffers);
    ...
    protected override void PropagateTaskCompletion()
    {
        if (HasLost())
        {
            ResetBallLocationProbabilities();
            EndEpisode();
        }
    }
    ...
    private void ResetBallLocationProbabilities()
    {
        InitializeBallLocationProbabilities(_numberOfBins, _platformRadius);
    }
    ...
    private void UpdateBallLocationProbabilities()
    {
        ...
        if (IsVisible)
        {
            ...
            _estimatedVelocity = _ballRb.linearVelocity * UpdatePeriod;
            _currentSigmaMean = SigmaMean;
        }
        else
        {
            //Vector3 currentVelocity = _ballRb.velocity * (UpdatePeriod + UpdatePeriod * UpdatePeriod);
            Vector3 currentVelocity = _ballRb.linearVelocity * UpdatePeriod;
