...
public class VisualStateSpace
{
    ...
    public ITask Task { 
        get 
        {
            return Camera.gameObject.transform.parent.GetChildInHierarchyByName("Agent").gameObject.GetComponent<ITask>();
        } 
    }
    ...
    public bool IsActiveElementAtScreenPosition(Vector2 screenPosition, float margin = 0)
    {
    ...
    public bool IsActiveElementAtViewportPosition(Vector2 viewportPosition, float margin = 0)
    {
        List<GameObject> gameObjects = GetGameObjectForViewportCoordinates(viewportPosition, margin);
    ...
    public Vector3 GetViewportCoordinatesForGameObjectIndex(int index)
    {
        return GetViewportPositionForWorldPosition(Camera, VisualElements[index].transform.position);
    }

    public Vector3 GetViewportCoordinatesForGameObjectName(string name)
    {
        return GetViewportPositionForWorldPosition(Camera, VisualElements.Find(a => a.name == name).transform.position);
    }

    public Vector3 GetViewportCoordinatesForGameObject(GameObject gameObject)
    {
        return GetViewportPositionForWorldPosition(Camera, gameObject.transform.position);
    }

    public Vector3 GetViewportPositionForWorldPosition(Camera camera, Vector3 worldPosition)
    {
        return camera.WorldToViewportPoint(worldPosition);
    }

    public GameObject GetGameObjectForViewportCoordinates(Vector2 viewportCoordinates)
    {
        Vector3 screenPoint = Camera.ViewportToScreenPoint(
            new Vector3(viewportCoordinates.x, viewportCoordinates.y, 0)
        );
        Vector2 screenCoordinates = new Vector2(screenPoint.x, screenPoint.y);

        return GetGameObjectForScreenCoordinates(screenCoordinates);
    }
    ...
    public List<GameObject> GetGameObjectForViewportCoordinates(Vector2 viewportCoordinates, float margin)
    {
        List<GameObject> gameObjects = new();
        ...
        foreach (GameObject uiElement in VisualElements)
        {
            ...
            // Get the object's world corners
            Vector3[] worldCorners = new Vector3[4];
            rectTransform.GetWorldCorners(worldCorners);

            // Convert world corners to viewport space
            Vector3 bottomLeftVP = Canvas.worldCamera.WorldToViewportPoint(worldCorners[0]);
            Vector3 topRightVP = Canvas.worldCamera.WorldToViewportPoint(worldCorners[2]);

            // Expand the rect by the margin (in viewport units!)
            Rect expandedRect = new Rect(
                bottomLeftVP.x - margin,
                bottomLeftVP.y - margin,
                (topRightVP.x - bottomLeftVP.x) + 2 * margin,
                (topRightVP.y - bottomLeftVP.y) + 2 * margin
            );

            // Check if the position is within this expanded rect
            if (expandedRect.Contains(viewportCoordinates))
            {
                gameObjects.Add(uiElement);
            }
        }
    ...
    {
        int index = VisualElements.IndexOf(gameObject);

        if (index != -1) ActivateElement(index);
    ...
    public bool DeactivateElement(GameObject gameObject)
    {
        ...
        if (index != -1) DeactivateElement(index);
    ...
    public Vector2 GetMaxScreenDistanceBetweenVisualElementsScreenCoordinates()
    {
        return GetMaxScreenDistanceBetweenVisualElements(GetScreenCoordinatesForGameObjectIndex);
    }

    public Vector2 GetMaxScreenDistanceBetweenVisualElementsViewport()
    {
        return GetMaxScreenDistanceBetweenVisualElements(GetViewportCoordinatesForGameObjectIndex);
    }
    ...
    private Vector2 GetMaxScreenDistanceBetweenVisualElements(Func<int, Vector3> func)
    {
        float maxX = 0;
        float maxY = 0;
        ...
        for (int i = 0; i < VisualElements.Count; i++)
        {
            ...
            {
                Vector2 distance = func(i) - func(j);
