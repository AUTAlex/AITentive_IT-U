...
public class ConfigVersioning
{
    public static List<ISettings> UnifySettings(string json, Type type)
    {
        ...
        catch (Exception)
        {
            ...
            try
            {
                members = JsonConvert.DeserializeObject<Dictionary<string, string>>(json);
            }
            catch (Exception)
            {
                members = FlattenJsonToStringDictionary(json);
            }
            ...
            {
                settings = new JsonAgentSettings(0, type, members, int.Parse(members["decisionPeriod"]), typeof(Task));
            }
    ...
    private static Dictionary<string, string> FlattenJsonToStringDictionary(string json)
    {
    ...
    /// <summary>
    /// Allows to define the supervisorChoice based on jsonSettings.type.Name.
    /// </summary>
