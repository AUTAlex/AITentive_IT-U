...
namespace Algorithms
{
    public static class TextDistance
    {
        ...
        public static double CalculateAccuracy(string reference, string typed)
        {
            if(typed.Length == 0 && reference.Length == 0)
            {
                return 1;
            }
            ...
            string target = reference.ToLower();
            string current = typed.ToLower();

            int levenshteinDistance = TextDistance.CalculateLevenshtein(target, current);

            int correctKeystrokes = target.Length - levenshteinDistance;

            correctKeystrokes = Mathf.Max(correctKeystrokes, 0);

            return ((double)correctKeystrokes / target.Length);
        }
        ...
        public static float GetRewardAfterProofreading(float collectedBeliefReward, string lastProofedText, string newText, string targetText)
        {
            ...
            int numberOfDeletions = GetNumberOfDeletions(newText, targetText) - GetNumberOfDeletions(lastProofedText, targetText);
