...
public static class Util
{
    public static string GetScoreString(SupervisorSettings supervisorSettings, Hyperparameters hyperparameters)
    {
        ...
        else if (supervisorSettings.supervisorChoice.ToLower() != "nosupport")
        {
    ...
    public static void SaveDataToCSV<T>(string path, List<T> data, bool overwritte = false)
    {
        try 
        {
            SaveToCSV(path, data, overwritte);
        }
        catch (Exception e)
        {
            string extension = Path.GetExtension(path);
            string backupPath = $"{Path.GetFileNameWithoutExtension(path)}{DateTime.Now.ToString("yyyyMMddHHmm")}{extension}";
            Debug.LogWarning($"Could not save file to path {path}: {e.Message}.\nSave file to {backupPath}.");

            SaveToCSV(backupPath, data, overwritte);
        }
    }
    ...
    public static T ImportJsonNewtonsoft<T>(string path)
    {
        if (!File.Exists(path))
        {
    ...
    /// <summary>
    /// Returns a list of random values that sum up to a constant total value.
    /// </summary>
    /// <param name="minValue"></param>
    /// <param name="maxValue"></param>
    /// <param name="minNumEntries"></param>
    /// <param name="maxNumEntries"></param>
    /// <param name="numEntries"></param>
    /// <returns></returns>
    /// <exception cref="ArgumentException"></exception>
    public static List<int> GenerateRandomValues(int minValue, int maxValue, int numEntries)
    {
        System.Random rnd = new();

        int totalBudget = (int)((((maxValue - minValue) / 2f) + minValue) * numEntries);

        if (numEntries * minValue > totalBudget || numEntries * maxValue < totalBudget)
        {
            throw new ArgumentException("It is not possible to distribute the total priority among the tasks with the given constraints.");
        }

        List<int> priorities = new List<int>(new int[numEntries]);

        int remainingPriority = totalBudget;

        for (int i = 0; i < numEntries; i++)
        {
            int maxAssignable = Math.Min(maxValue, remainingPriority - (numEntries - i - 1) * minValue);
            int minAssignable = Math.Max(minValue, remainingPriority - (numEntries - i - 1) * maxValue);

            priorities[i] = rnd.Next(minAssignable, maxAssignable + 1);
            remainingPriority -= priorities[i];
        }

        return priorities.OrderBy(x => rnd.Next()).ToList(); // Shuffle to randomize order
    }

    public static float Entropy(double[] probs)
    {
        double sum = probs.Sum();
        if (sum <= 0.0)
            return 0f; // or throw, depending on your preference

        float entropy = 0f;

        for (int i = 0; i < probs.Length; i++)
        {
            // normalize each probability without modifying the input array
            float p = (float)(probs[i] / sum);

            if (p <= 0f)
                continue; // contributes 0 to entropy

            float summand = p * MathF.Log(p, 2); // log base 2

            if (float.IsNaN(summand))
            {
                UnityEngine.Debug.LogWarning($"Bin {i} with probability {p} leads to NaN value.");
                continue;
            }
        ...
        // entropy is <= 0; return positive entropy in bits
        return -entropy; // [0, log2(N)]
    }

    public static float Entropy01(double[] probs)
    {
        ...
        float maxEntropy = MathF.Log(n, 2);
    ...
    private static void SaveToCSV<T>(string path, List<T> data, bool overwritte = false)
    {
    ...
    private static void WriteToCSV<T>(string path, TypeConverterOptions options, List<T> data, FileMode mode)
    {
        ...
        {
            Debug.LogWarning($"No data to write to CSV {path}.");
            return;
        }
    ...
    private static void WriteHeaderToCSV<T>(CsvWriter csv, List<T> data, string prefix = "")
    {
        ...
        foreach (var thisVar in members)
        {
            if (thisVar.GetUnderlyingType() == typeof(Dictionary<string, object>) || thisVar.GetUnderlyingType() == typeof(Dictionary<string, double>) || thisVar.GetUnderlyingType() == typeof(Dictionary<string, string>))
            {
                switch(thisVar.GetUnderlyingType())
                {
                    ...
                    case Type t when t == typeof(Dictionary<string, double>):
                        ...
                        break;
                    case Type t when t == typeof(Dictionary<string, string>):
                        WriteDictionaryToHeader<T, string>(csv, thisVar, data);
                        break;
                }
            }
    ...
    private static void WriteDictionaryToHeader<T1, T2>(CsvWriter csv, MemberInfo thisVar, List<T1> data)
    {
        ...
        var values = records.Select(x => thisVar.GetValue(x));
        ...
        Dictionary<string, T2> value = values.First() as Dictionary<string, T2>;
