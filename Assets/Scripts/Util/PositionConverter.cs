...
public static class PositionConverter
{
    ...
    {
        List<int> result = new() { bin };

        float binSize = (rangeMax - rangeMin) / numberOFBins;

        int numberOfCrossedBins = (int)Math.Round((Math.Abs(velocity) / binSize));
    ...
    public static float ContinuousValueToBinCenter(float value, float range, int numberOfBins)
    {
        int bin = ContinuousValueToBin(value, range, numberOfBins);

        return BinToContinuousValue(bin, range, numberOfBins);
    }
    ...
    public static int ContinuousValueToBinBurst(float value, float min, float max, int numberOfBins)
    {
        ...
        int bin = (int)(((value - min) / range) * numberOfBins);
        if (bin == numberOfBins)
            bin = numberOfBins - 1;
    ...
    public static float ContinuousValueToBinCenter(float value, float min, float max, int numberOfBins)
    {
        int bin = ContinuousValueToBin(value, min, max, numberOfBins);

        return BinToContinuousValue(bin, min, max, numberOfBins);
    }
