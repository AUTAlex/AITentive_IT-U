...
public class BehaviorMeasurementBehavior : MonoBehaviour
{
    ...
    private void OnEnable()
    {
        ...
        {
            SupervisorAgent = SupervisorAgent.GetSupervisor();
            try
            {
            ...
            {
                Debug.LogWarning($"Behavioral data logging failed because some tasks do not implement the required methods. {e.Source}: {e.Message}");
                return;
            }
...
public class BehaviorMeasurement
{
    ...
    public void UpdateActiveInstance(double timeBetweenSwitches, int targetTask = default, bool isNewEpisode = false, bool wasDecisionRequestedBySystem = false, Switcher switcher = default)
    {
    ...
    private void CollectBehavioralData(List<dynamic> performedActions, ITask task)
    {
        ...
        if (!_isReactionTimeMeasurementActive)
        {
            Dictionary<int, (int, List<(dynamic, dynamic)>)> averagePerformedActionsDiscretizedSpace = null;

            try 
            {
                averagePerformedActionsDiscretizedSpace = (Dictionary<int, (int, List<(dynamic, dynamic)>)>)task.StateInformation.AveragePerformedActionsDiscretizedSpace.GetValue(bins[..^1]);
            }
            catch (IndexOutOfRangeException e)
            {
                Debug.LogError($"Index out of range for array with rank {task.StateInformation.AveragePerformedActionsDiscretizedSpace.Rank} and bin {String.Join("; ", bins[..^1])}: {e.Message}");
            }
