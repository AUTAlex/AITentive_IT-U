...
def Main():
    ...
    parser.add_argument("--task_agent_models_dir_name", help="Name of models directory in Assets/Models path which should be evaluated.", type=str)
    parser.add_argument("--model_prefix", help="Prefix of model used in asset file (e.g. \"TypingAgent\" in \"TypingAgentH2_AUIMessagingH2_AUI_2024_TaskQueue_Typing\").", type=str)
    parser.add_argument("--environment_config_list_file", "-e",
        help="Config file in JSON format consisting of lists of the different parameters to generate the environment config files. Replaces config" 
            +"files of task_agent_models_dir_name if given.", type=str)
    ...
    parser.add_argument("--target_dir", "-d", help="If given the results are saved to the target- instead of the session directory.", type=str)
    parser.add_argument("--time_scale", "-t", help="The time scale of the Unity environment(s). Equivalent to setting Time.timeScale in Unity.", type=int)
    ...
    if args.nobuild is None:
        if args.task_agent_models_dir_name is None:
            session_dir = __build_evaluation_environment(evaluation_config_file = args.evaluation_config_file, 
                                                         comparison_file_name = comparison_file_name,
                                                         time_scale=time_scale)
        elif args.environment_config_list_file is None:
            session_dir = __build_all_evaluation_environments(task_agent_models_dir_name = args.task_agent_models_dir_name,
                                                              evaluation_config_file = args.evaluation_config_file, 
                                                              comparison_file_name = comparison_file_name,
                                                              time_scale=time_scale)
        else:
            session_dir = __build_all_evaluation_environments_with_config_list(task_agent_models_dir_name = args.task_agent_models_dir_name,
                                                                               evaluation_config_file = args.evaluation_config_file,
                                                                               comparison_file_name = comparison_file_name,
...
def __build_evaluation_environment(evaluation_config_file, comparison_file_name, time_scale=20):
    
    call_function("BuildScript.BuildEvaluationEnvironment",
        Path(evaluation_config_file).stem, #e.g. Session1
        Path("config", evaluation_config_file), #e.g. Assets\Models\Session1\model_name\envConf.json
        " ",
        Path("config", evaluation_config_file), 
        time_scale,
        comparison_file_name,
        logFile=Path("..", "Logs", "Builds", Path(evaluation_config_file).stem, "LogFileBuild.txt"))
    
    return Path(evaluation_config_file).stem


# Builds an own environment for each model inside of ball_agent_models_dir_name.
def __build_all_evaluation_environments(task_agent_models_dir_name, evaluation_config_file, comparison_file_name, time_scale=20):
    model_dir = Path('..', 'Assets', 'Models', task_agent_models_dir_name)

    sub_dirs = [d for d in model_dir.glob('*') if d.is_dir()]

    print(task_agent_models_dir_name)

    if not sub_dirs:
        sub_dirs.append(task_agent_models_dir_name)
    ...
    for dir in sub_dirs:
        ...
        try:
            ...
            call_function("BuildScript.BuildEvaluationEnvironment",
                task_agent_models_dir_name, #e.g. Session1
                json_path, #e.g. Assets\Models\Session1\model_name\envConf.json
                Path(task_agent_models_dir_name, dir.name, asset_filename), #e.g. Session1\model_name\model.asset
                Path("config", evaluation_config_file), 
                time_scale,
                comparison_file_name,
                logFile=Path("..", "Logs", "Builds", task_agent_models_dir_name, dir.name,"LogFileBuild.txt"))
    ...
    return task_agent_models_dir_name


#Builds an own model for every parameter combination but with the same model (or set of models with different decision periods handled by the Unity implementation)
def __build_all_evaluation_environments_with_config_list(task_agent_models_dir_name, evaluation_config_file, comparison_file_name, environment_config_list_file, number_of_environments, start_index=0, time_scale=20):
    ...
    for conf_file in conf_files:
        ...
        returncode = call_function("BuildScript.BuildEvaluationEnvironment",
                                   ...
                                   task_agent_models_dir_name, #e.g. model_name\model.asset
                                   Path("config", evaluation_config_file), 
                                   time_scale,
                                   comparison_file_name,
                                   logFile=Path("..", "Logs", "Builds", task_agent_models_dir_name, "LogFileBuild.txt"))
