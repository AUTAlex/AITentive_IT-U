...
def bolfi(d, model_path, isVerbose):
    ...
    acq_noise_var={'sigma':0.01, 'sigmaMean':0.01, 'updatePeriode':0.01, 'observationProbability':0.001, 'constantReactionTime':0.01, 'oldDistributionPersistenceTime':0.01, 'decisionPeriodBallAgent': 0}
...
def fetchResults(procs, resultList, active_instances):
    ...
    for id, p in procs.copy().items():
        if p.poll() != None:
            ...
            try:
                if is_bin_data:
                    resultReactionTimePath = Path(buildPath, "SupervisorML_Data", "Scores", "{0}rt_simBSI.json".format(id))
                    resultDurationPath = Path(buildPath, "SupervisorML_Data", "Scores", "{0}sim_scores.csv".format(id))
                    resultActionPath = Path(buildPath, "SupervisorML_Data", "Scores", "{0}behavior_simBSI.json".format(id))
