...
- **(F)** General Settings
  - Mode - Specifies the conditions applied during the user study.
    - Force - The AMS controls the switches, and the user must follow the changes.
    - Notification - Switches are suggested by the supervisor model, but require user confirmation.
    - Suggestion - Switches are recommended by the supervisor model, though the user retains control over the active task.
  ...
  - Game Mode - The tasks are solved by the user if true, otherwise by the task-agent. 
